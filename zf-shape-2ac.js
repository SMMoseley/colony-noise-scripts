...
function block34_peck2(key) {
    ...
    const position = pick_random_element(["left", "center"]);
    const cue = "cue_" + position + "_" + par.cue_color;
    const position_key = "peck_" + position;

    t.change_state(cue, { trigger: null, brightness: 1})

    update_state({ phase: "awaiting-response-2"});
    t.await("keys", null, function(msg) { return msg[position_key]}, _exit);
